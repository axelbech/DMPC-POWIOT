...
class DMPCCoordinator():
    ...
    def run_full(
        ...
        self.f_sum_last = 1e6
        f_tol = 1e-5 * self.N# 1e-4 * self.N # 1e-3 * self.N # 5e-2 * self.N # 1e-3 * self.N
        dv_tol = 1e-5 # 1e-4 # 1e-3 # 5e-2 # 1e-4 # 1e-3
        ...
        while t < self.T:
            ...
            self.iterate_dual_variables() #comment away if using hourly cost
            public_coordination['dual_variables'] = self.dual_variables
